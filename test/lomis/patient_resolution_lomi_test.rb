...
module Chla
  class PatientResolutionLomiTest < Test::Unit::TestCase
    ...
    def populate_test_data
      ...
      @alice_picudb_encounter_id = @lomi.encounters.insert({:patient_id => @alice_picudb_id, :source => "picudb", :admission => Time.new(2000, 12, 25), :discharge => Time.new(2000, 12, 28)})
      ...
      @alice_cerner_event_id = @lomi.events.insert({:patient_id => @alice_cerner_id, :encounter_id => @alice_cerner_encounter_id, :start_time => Time.new(2000, 12, 26), :end_time => Time.new(2000, 12, 26) })
    end
    ...
    def test_propagate_resolved_patient_ids
      ...
      puts @alice_picudb_encounter_id
      puts @lomi.encounters.find().to_a
    ...
    def test_resolve_encounter_ids
      resolved_patient_id = SecureRandom.hex
      @lomi.patients.update({mrn: @alice_mrn}, {:$set => {:resolved_patient_id => resolved_patient_id}}, {:multi => true})
      @lomi.encounters.update({:_id => @alice_picudb_encounter_id}, {:$set => {:resolved_patient_id => resolved_patient_id}})
      @lomi.encounters.update({:_id => @alice_cerner_encounter_id}, {:$set => {:resolved_patient_id => resolved_patient_id}})
      @lomi.events.update({:_id => @alice_picudb_event_id}, {:$set => {:resolved_patient_id => resolved_patient_id}})
      @lomi.events.update({:_id => @alice_cerner_event_id}, {:$set => {:resolved_patient_id => resolved_patient_id}})
      assert_equal @lomi.events.find({:resolved_encounter_id => {:$exists => true}}).count(), 0
      @lomi.resolve_encounter_ids
      assert_equal @lomi.events.find_one({:_id => @alice_cerner_event_id})["resolved_encounter_id"], @lomi.encounters.find_one({:_id => @alice_cerner_encounter_id})["_id"]
      assert_equal @lomi.events.find({:_id => @alice_picudb_event_id, :resolved_encounter_id => {:$exists => true}}).count(), 0
    end
  end
end
